...
const ConnectWallect = () => {
  ...
  const providerEth = new ethers.providers.JsonRpcProvider(
    "https://distinguished-lingering-energy.quiknode.pro/7658cc525cd569fa1968f876c7b7a579906ae37f/"
  );
  const providerBSC = new ethers.providers.JsonRpcProvider(
  ...
  const bscSquidTokenAddress = "0xd8fa690304d2b2824d918c0c7376e2823704557a"; //SquidGrow token contract address on BSC
  const ethSquidTokenAddress = "0xd8fa690304d2b2824d918c0c7376e2823704557a"; //SquidGrow token contract address on ETH
  // const bscDecimals = 9;
  // const ethDecimals = 18;

  const bscTokenDecimals = 9;
  const ethTokenDecimals = 9;
  ...
  const oftWrapSquidGrowETH = "0x44b9f1c5c9c233d8f21a4ef45b020037fedc2c6c"; // wrap token on ETH
  const oftWrapSquidGrowBSC = "0x44b9f1c5c9c233d8f21a4ef45b020037fedc2c6c"; // wrao token on BSC
    ...
    try {
      if (chain.id == bscChainId || chainId == bscChainId) {
        ...
        const tx = await squidgrow.approve(
          oftWrapSquidGrowBSC,
          ethers.utils.parseUnits(inputValue.toString(), bscTokenDecimals)
        );
        ...
        const tx = await squidgrow.approve(
          oftWrapSquidGrowETH,
          ethers.utils.parseUnits(inputValue.toString(), ethTokenDecimals)
        );
  ...
  const fetchApproval = async () => {
    if (chain.id == bscChainId) {
      ...
      const allowanceFormatted = parseFloat(
        ethers.utils.formatUnits(allowance, bscTokenDecimals)
      );
      ...
      const allowanceFormatted = parseFloat(
        ethers.utils.formatUnits(allowance, ethTokenDecimals)
      );
  ...
  const handleBridge = async (input) => {
    ...
    if (chain?.id == bscChainId) {
      ...
      const fees = await oftWrapContract.estimateSendFee(
        ethRemoteChainId,
        address,
        amount,
        false,
  ...
  const getBridgeBalance = async () => {
    if (chain?.id == bscChainId) {
      ...
      const formatted = parseFloat(
        ethers.utils.formatUnits(balance, bscTokenDecimals)
      ).toFixed(0);
      ...
      const formatted = parseFloat(
        ethers.utils.formatUnits(balance, ethTokenDecimals)
      ).toFixed(0);
